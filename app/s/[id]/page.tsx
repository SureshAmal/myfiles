...
export default function SharePage({
  ...
  const displayFiles: DisplayFile[] = useMemo(() => {
    if (!shareData) return [];
    if (downloadLinks) return downloadLinks.map((dl) => ({ ...dl }));
    return shareData.files.map((f) => ({ ...f, url: null }));
  }, [shareData, downloadLinks]);

  const downloadedCount = downloadedIds.length;
  const totalCount = displayFiles.length;
  const allDownloaded =
    isUnlocked && totalCount > 0 && downloadedCount === totalCount;
    ...
    <main className="min-h-screen lg:h-screen lg:overflow-hidden flex flex-col bg-background font-sans">
      ...
      <div className="flex flex-col lg:flex-row flex-1 lg:min-h-0 lg:overflow-hidden">
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  <span>Progress</span>
                  <span className={allDownloaded ? "text-accent" : ""}>
                ...
                <div className="h-1.5 w-full bg-muted border border-border overflow-hidden">
                  <div
                    className={`h-full transition-all duration-300 ${allDownloaded ? "bg-accent" : "bg-primary"}`}
                    style={{
                      width: totalCount
              ...
              <p className="text-[10px] font-mono text-muted-foreground/60 uppercase tracking-wider leading-relaxed border-t-2 border-border/40 pt-4">
                Presigned links expire in 1 hour. Refresh the page and re-enter
        ...
        <div className="flex-1 flex flex-col lg:overflow-hidden">
          {/* Toolbar */}
          <div className="shrink-0 border-b-2 border-border px-5 py-2.5 flex items-center justify-between gap-4 bg-muted/10">
            <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground font-mono">
              {isUnlocked
          ...
          <div className="flex-1 lg:overflow-y-auto">
            {displayFiles.length === 0 ? (
              <div className="flex items-center justify-center h-full p-10 text-muted-foreground/30">
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 z-10 bg-background">
                  <tr className="border-b-2 border-border">
                    <th className="text-left px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-10 tabular-nums">
                      #
                    </th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      File
                    </th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground hidden md:table-cell w-40">
                      Type
                    </th>
                    <th className="text-right px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-24">
                      Size
                    </th>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 p-5">
                {displayFiles.map((file) => {
                  ...
                  return (
                    <div
                      key={file.id}
                      className={`flex flex-col border-2 border-border transition-colors ${
                        locked
                          ? "opacity-60"
                          : "hover:bg-muted/20 hover:border-primary"
                      }`}
                    >
                      {/* Card body */}
                      <div className="flex flex-col items-center gap-2 p-4 flex-1 text-center">
                        ...
                        <span
                          className={`font-bold text-xs leading-snug break-all line-clamp-2 w-full ${
                            locked
                              ? "text-muted-foreground/50"
                              : "text-foreground"
                          }`}
